...
## Where Booking Pages Appear on Your Public Site

| Public URL | What Visitors See |
|---|---|
| `yoursite.com/book/{slug}` | The event type details and available time slots for booking. |

- The `{slug}` is set when you create the event type in admin (auto-generated from the name).
- Visitors see a date picker, available time slots, and a form to submit their name, email, and notes.
- The calendar must be **active** for the booking page to accept new bookings.

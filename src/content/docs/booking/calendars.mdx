...
Each event type under a calendar has its own public-facing booking page at:

```
yoursite.com/book/{event-type-slug}
```

This is the page visitors use to view event details, pick a date, select an available time slot, and submit their booking. Behind the scenes, the booking page calls the API endpoints (`/api/book/{event-type-slug}` and `/api/book/{event-type-slug}/slots`) to fetch availability and submit appointments.

...
The Settings page is organized into six tabs:

1. **General** -- core site identity (name, tagline, logo, favicon, footer)
2. **Branding** -- visual customization (fonts and colors), covered in the [Branding](/docs/settings/branding) page
3. **Social** -- links to your social media profiles
4. **SEO** -- default meta tags for search engines
5. **Integrations** -- Google Calendar, Zoom, and meeting provider configuration, covered in the [Integrations](/docs/settings/integrations) page
6. **System** -- runtime information, enabled services, and version details, covered in the [System Info](/docs/settings/system-info) page
...
Settings from the General, Branding, Social, and SEO tabs are applied to your public website at `yoursite.com` through the **theme API**. When the public site (the `web` app) renders a page, it fetches the current settings from the API and uses them across every page:
...
| **Branding** | The brand font and primary color from the Branding tab affect typography and accent colors across the entire site. See [Branding](/docs/settings/branding) for details. |
...
| **Integrations** | Google Calendar sync and meeting provider (Google Meet / Zoom) for the Booking module. See [Integrations](/docs/settings/integrations). |
| **System** | Read-only runtime info: version, Go runtime, database, enabled services. See [System Info](/docs/settings/system-info). |

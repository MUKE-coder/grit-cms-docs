...
## Where Products Appear on Your Public Site

When you create a product in the admin dashboard, it becomes available on your public-facing storefront at the following URLs:

- **Product catalog** -- `yoursite.com/products` lists all **active** products. Each product is displayed with its name, featured image, price, and type so visitors can browse your full catalog at a glance.
- **Individual product page** -- `yoursite.com/products/{slug}` shows the complete product detail page, including the full description, all images, every available price tier, variants, and a **Buy** button for checkout.

The `{slug}` is the URL-friendly identifier you set (or auto-generate from the name) when creating the product in admin. For example, a product named "Premium Ebook" with the slug `premium-ebook` would be available at `yoursite.com/products/premium-ebook`.

Only products with an **active** status appear publicly. Products set to **inactive** or **archived** are hidden from the storefront entirely.

For **course**-type products, the buyer is automatically enrolled in the linked course upon purchase. After payment, they can access the course at `yoursite.com/courses/{course-slug}`.
...
Only products with an **active** status appear on your public storefront at `yoursite.com/products`. Visitors can browse the full product catalog, view individual product pages at `yoursite.com/products/{slug}`, and see all available prices and variants.
...
Each active product has a public page accessible at `yoursite.com/products/{slug}`. The product page displays the product name, description, images, all available price tiers, and any variants. Customers can select a variant and price tier before proceeding to checkout.

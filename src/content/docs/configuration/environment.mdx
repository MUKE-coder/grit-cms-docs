...
GritCMS uses [Stripe](https://stripe.com) for payment processing. When configured, customers can pay for products and courses using Stripe Elements (credit cards, Apple Pay, Google Pay, etc.) directly on your site.
...
Get your keys from the [Stripe Dashboard](https://dashboard.stripe.com/apikeys). For webhooks, create a webhook endpoint pointing to `https://api.yourdomain.com/api/webhooks/stripe` and subscribe to `payment_intent.succeeded` and `payment_intent.payment_failed` events.

If these variables are not set, payment features are disabled and only free courses/products are available.
...
- Configured `STRIPE_SECRET_KEY` and `STRIPE_PUBLISHABLE_KEY` for payment processing
- Set up a Stripe webhook with `STRIPE_WEBHOOK_SECRET` for reliable order fulfillment
